...
dp = Dispatcher()
subscribers = []  # Список подписчиков (в памяти)
client_requests = []  # Список заявок от клиентов

print(f"📊 Подписчиков в памяти: {len(subscribers)}")
print(f"📋 Заявок в памяти: {len(client_requests)}")
...
async def start_command(message: Message):
    ...
    if user_id not in subscribers:
        ...
        print(f"➕ Новый подписчик: {user_name} (ID: {user_id})")

    # Клавиатура для нового подписчика
...
async def unsubscribe_command(message: Message):
    ...
    if user_id in subscribers:
        ...
        print(f"➖ Отписался: ID {user_id}")
    else:
        await message.answer("ℹ️ Вы не были подписаны.")

print("✅ Команды для пользователей добавлены!")
...
async def admin_command(message: Message):
    ...
    keyboard = types.ReplyKeyboardMarkup(
        keyboard=[
            ...
            [types.KeyboardButton(text="🛍️ Пример акции")],
...
@dp.message(lambda message: message.text == "🛍️ Пример акции" and message.from_user.id == ADMIN_ID)
async def promo_example_button(message: Message):
    """Пример акции с inline-кнопками"""
    keyboard = types.InlineKeyboardMarkup(
        inline_keyboard=[
            [types.InlineKeyboardButton(text="📋 АЛГОРИТМ ПОЛУЧЕНИЯ УСЛУГИ", callback_data="techplan_process")],
            [types.InlineKeyboardButton(text="📞 ОСТАВИТЬ ЗАЯВКУ", callback_data="leave_request")],
...
async def techplan_process_handler(callback_query: types.CallbackQuery):
    """Обработка кнопки с порядком изготовления техплана"""
    process_text = """
📋 <b>ПОРЯДОК ПОЛУЧЕНИЯ УСЛУГИ:</b>

1️⃣ <b>Изучение документов</b> и заключение договора
2️⃣ <b>Выезд на объект</b> для обмеров
3️⃣ <b>Подготовка документов (Технический план здания)</b>
4️⃣ <b>Передача заказчику технического плана на CD диске</b>


⏱️ <b>Срок исполнения:</b> 2-3 рабочих дня
"""
...
async def write_to_client_handler(callback_query: types.CallbackQuery):
    ...
    try:
        await bot.send_message(
            chat_id=int(user_id),
            text="👷‍♂️ <b>Сообщение от кадастрового инженера</b>\n\n"
...
async def take_request_handler(callback_query: types.CallbackQuery):
    """Кнопка "Взять в работу" в уведомлении"""
    if callback_query.from_user.id != ADMIN_ID:
        await callback_query.answer("⛔ Доступ запрещен", show_alert=True)
        return
...
print("✅ Панель администратора добавлена!")
...
async def handle_admin_mailing(message: Message):
    ...
    if message.text in ["📊 Статистика", "📢 Сделать рассылку", "🛍️ Пример акции", "❌ Закрыть панель"]:
        return
...
"""
    ...
    # Показываем статистику в консоли Colab
    print("=" * 50)
...
print("✅ Система рассылки готова!")

async def main():
    print("=" * 60)
    print("🚀 ЗАПУСКАЕМ БОТА КАДАСТРОВОГО ИНЖЕНЕРА")
    print("=" * 60)
    ...
    try:
        ...
        print("Проверьте токен бота в Ячейке 2")
        return

    print("\n" + "=" * 60)
    print("📱 ИНСТРУКЦИЯ ДЛЯ РАБОТЫ:")
    print("=" * 60)
    print("1️⃣ Откройте Telegram на телефоне/компьютере")
    print("2️⃣ Найдите бота: @my_kaddelo_bot")
    print("3️⃣ Напишите боту команду /start (чтобы подписаться)")
    print("\n4️⃣ Откройте бота со своего аккаунта (админа)")
    print("5️⃣ Напишите команду /admin")
    print("6️⃣ Используйте кнопки для управления:")
    print("   📊 Статистика - посмотреть подписчиков и заявки")
    print("   📢 Сделать рассылку - отправить сообщение всем")
    print("   🛍️ Пример акции - посмотреть шаблон с кнопками")
    print("   ❌ Закрыть панель - убрать кнопки")
    print("\n" + "=" * 60)
    print("⚠️  ВАЖНО: Бот работает пока открыт Colab")
    print("⏹️  Для остановки: Нажмите Stop в Colab")
    print("=" * 60 + "\n")
...
print("🎯 Нажмите ▶️ чтобы запустить бота...")
print("После запуска перейдите в Telegram")

# Запуск в Colab с использованием nest_asyncio
